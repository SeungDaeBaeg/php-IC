...
<div id="memo_list" class="new_win">
    <h1 id="win_title">
    	<?=$g5['title'] ?>
    	<div class="win_total">전체 <?=$kind_title ?>쪽지 <?=$total_count ?>통<br></div>
    </h1>
    <div class="new_win_con2">
            ...
            <ul>       	
	            ...
	            <li class="<?=$readed; ?>">
	                ...
	                <div class="memo_li memo_name">
	                	<?=$list[$i]['name']; ?> <span class="memo_datetime"><i class="fa fa-clock-o" aria-hidden="true"></i> <?=$list[$i]['send_datetime'] ?></span>
						<div class="memo_preview">
						    <a href="<?=$list[$i]['view_href'] ?>"><?=$memo_preview; ?></a>
                        </div>
					</div>	
	                <?php /* 쪽지 읽은 시간 echo $list[$i]['read_datetime']; */ ?>	
					<a href="<?=$list[$i]['del_href'] ?>" onclick="del(this.href); return false;" class="memo_del"><i class="fa fa-trash-o" aria-hidden="true"></i> <span class="sound_only">삭제</span></a>
	            </li>
	            <?php }  ?>
        ...
        <p class="win_desc"><i class="fa fa-info-circle" aria-hidden="true"></i> 쪽지 보관일수는 최장 <strong><?=$config['cf_memo_del'] ?></strong>일 입니다.
        </p>
