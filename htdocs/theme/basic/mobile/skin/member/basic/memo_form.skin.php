        ...
        <div class="form_01">
            ...
            <ul>
                <li>
                    ...
                    <input type="text" name="me_recv_mb_id" value="<?=$me_recv_mb_id; ?>" id="me_recv_mb_id" required class="frm_input full_input required" size="47" placeholder="받는 회원아이디">
                    <span class="frm_info">여러 회원에게 보낼때는 컴마(,)로 구분하세요.
                    	<?php if ($config['cf_memo_send_point']) { ?><br>쪽지 보낼때 회원당 <?=number_format($config['cf_memo_send_point']); ?>점의 포인트를 차감합니다.<?php } ?>
                    </span>
                </li>
                <li>
                    <label for="me_memo" class="sound_only">내용</label>
                    <textarea name="me_memo" id="me_memo" required class="required"><?=$content ?></textarea>
                </li>
                <li>
