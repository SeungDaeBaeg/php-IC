...
<div id="poll_result" class="new_win">
    <h1 id="win_title"><i class="fa fa-bar-chart" aria-hidden="true"></i> <?=$g5['title'] ?></h1>
    <div class="new_win_con">
        <!-- 설문조사 결과 그래프 시작 { -->
        <span class="poll_all">전체 <?=$nf_total_po_cnt ?>표</span>
        <section id="poll_result_list">
            ...
            <?php for ($i=1; $i<=count($list); $i++) {  ?>
                <li>
                    ...
                    <span class="poll_percent"><?=number_format($list[$i]['rate'], 1) ?> %</span>
                    <div class="poll_result_graph">
                        <span style="width:<?=number_format($list[$i]['rate'], 1) ?>%"><strong class="poll_cnt"><?=$list[$i]['cnt'] ?> 표</strong></span>
                    </div>
                </li>
            <?php }  ?>
            </ol>
        ...
        <section id="poll_result_cmt">
            ...
            <article>
                <header>
                    <h2><?=$list2[$i]['pc_name'] ?><span class="sound_only">님의 의견</span></h2>
                    <?=$list2[$i]['name'] ?>
                    <span class="poll_datetime"><i class="fa fa-clock-o" aria-hidden="true"></i> <?=$list2[$i]['datetime'] ?> <span class="poll_cmt_del"><?php if ($list2[$i]['del']) { echo $list2[$i]['del']."<i class=\"fa fa-times-circle\" aria-hidden=\"true\"></i><span class=\"sound_only\">삭제</span></a>"; }  ?></span></span>
                </header>
                <p>
                    <?=$list2[$i]['idea'] ?>
                </p>
        ...
        <aside id="poll_result_oth">
            ...
            <ul>
                ...
                <li><a href="./poll_result.php?po_id=<?=$list3[$i]['po_id'] ?>&amp;skin_dir=<?=urlencode($skin_dir); ?>"> <?=$list3[$i]['subject'] ?> </a><span><i class="fa fa-clock-o" aria-hidden="true"></i> <?=$list3[$i]['date'] ?></span></li>
                <?php }  ?>
            </ul>
        </aside>
        <!-- } 설문조사 다른 결과 보기 끝 -->
