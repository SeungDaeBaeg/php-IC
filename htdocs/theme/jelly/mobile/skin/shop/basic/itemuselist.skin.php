...
<div id="sps_sch">
    <div class="sch_wr">
        ...
        <input type="text" name="stx" value="<?=$stx; ?>" id="stx" required class="frm_input" size="15">
...
<div id="sps">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        ...
        $row2 = sql_fetch(" select it_name from g5_shop_item where it_id = '{$row['it_id']}' ");
    ...
    <li>
        ...
        <div class="sps_img">
            <a href="<?=$it_href; ?>">
                <?=get_itemuselist_thumbnail($row['it_id'], $row['is_content'], 70, 70); ?>
                <span><?=$row2['it_name']; ?></span>
            </a>
        </div>

        <section class="sps_section">
            <h2><?=get_text($row['is_subject']); ?></h2>

            <dl class="sps_dl">
                <dt>평가점수</dt>
                <dd class="sps_star"><img src="<?=G5_SHOP_URL; ?>/img/s_star<?=$star; ?>.png" alt="별<?=$star; ?>개" width="80"></dd>
                <dt>작성자</dt>
                <dd><i class="fa fa-user" aria-hidden="true"></i> <?=get_text($row['is_name']); ?></dd>
                <dt>작성일</dt>
                <dd><i class="fa fa-clock-o" aria-hidden="true"></i> <?=substr($row['is_time'],2,8); ?></dd>

            </dl>

            <div id="sps_con_<?=$i; ?>" style="display:none;">
                <?=$is_content; // 사용후기 내용 ?>
            ...
            <div class="sps_con_btn"><button class="sps_con_<?=$i; ?>">내용보기 <i class="fa fa-caret-down" aria-hidden="true"></i></button></div>
        </section>
