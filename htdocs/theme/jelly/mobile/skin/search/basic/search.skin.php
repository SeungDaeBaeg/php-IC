...
<fieldset id="sch_res_detail">
    ...
    <div class="sch_wr">
        ...
        <input type="text" name="stx" id="stx" value="<?=$text_stx ?>" class="frm_input" required  maxlength="20">
...
<div id="sch_result">
    ...
    <section id="sch_res_ov">
        <h2><strong class="sch_word"><?=$stx ?></strong> 전체검색 결과</h2>
        <dl>
            <dt>게시판</dt>
            <dd><strong><?=$board_count ?>개</strong></dd>
            <dt>게시물</dt>
            <dd><strong><?=number_format($total_count) ?>개</strong></dd>
        </dl>
        <p><?=number_format($page) ?>/<?=number_format($total_page) ?> 페이지 열람 중</p>
    </section>
    <?php
        }
    }
    ...
    for ($idx=$table_index, $k=0; $idx<count($search_table) && $k<$rows; $idx++) {
     ?>
        <h2><a href="./board.php?bo_table=<?=$search_table[$idx] ?>&amp;<?=$search_query ?>"><?=$bo_subject[$idx] ?> 게시판 내 결과</a></h2>
        <ul>
        <?php
         ...
         ?>
            <li>
                <div  class="sch_res_title">
                    ...
                    <a href="<?=$list[$idx][$i]['href'] ?><?=$comment_href ?>" target="_blank" class="sch_res_new"><i class="fa fa-share-square-o" aria-hidden="true"></i><span class="sound_only">새창</span></a>
                </div>
        ...
        <div class="sch_more"><a href="./board.php?bo_table=<?=$search_table[$idx] ?>&amp;<?=$search_query ?>"><i class="fa fa-plus-circle" aria-hidden="true"></i> <strong><?=$bo_subject[$idx] ?></strong> 결과 더보기</a></div>
