...
$sql = " select ca_skin_dir, ca_include_head, ca_include_tail, ca_cert_use, ca_adult_use from g5_shop_category where ca_id = '{$it['ca_id']}' ";
$ca = sql_fetch($sql);
...
if (get_cookie('ck_it_id') != $it_id) {
    sql_query(" update g5_shop_item set it_hit = it_hit + 1 where it_id = '$it_id' "); // 1증가
...
// 이전 상품보기
$sql = " select it_id, it_name from g5_shop_item where it_id > '$it_id' and SUBSTRING(ca_id,1,4) = '".substr($it['ca_id'],0,4)."' and it_use = '1' order by it_id asc limit 1 ";
$row = sql_fetch($sql);
...
// 다음 상품보기
$sql = " select it_id, it_name from g5_shop_item where it_id < '$it_id' and SUBSTRING(ca_id,1,4) = '".substr($it['ca_id'],0,4)."' and it_use = '1' order by it_id desc limit 1 ";
$row = sql_fetch($sql);
if (isset($row['it_id']) && $row['it_id']) {
...
if($default['de_rel_list_use']) {
    $sql = " select count(*) as cnt from {$g5['g5_shop_item_relation_table']} a left join g5_shop_item b on (a.it_id2=b.it_id) where a.it_id = '{$it['it_id']}' and  b.it_use='1' ";
    $row = sql_fetch($sql);
...
?>
    <ul class="sanchor">
        ...
        <li><a href="#sit_use" <?php if ($anc_id == 'use') echo 'class="sanchor_on"'; ?>>사용후기 <span class="item_use_count"><?=$item_use_count; ?></span></a></li>
        <li><a href="#sit_qa" <?php if ($anc_id == 'qa') echo 'class="sanchor_on"'; ?>>상품문의 <span class="item_qa_count"><?=$item_qa_count; ?></span></a></li>
