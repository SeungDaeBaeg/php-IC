...
<section id="anc_sodr_list">
    ...
    <div class="local_desc02 local_desc">
        <p>
            현재 주문상태 <strong><?=$od['od_status'] ?></strong>
            |
            주문일시 <strong><?=substr($od['od_time'],0,16); ?> (<?=get_yoil($od['od_time']); ?>)</strong>
            |
            주문총액 <strong><?=number_format($od['od_cart_price'] + $od['od_send_cost'] + $od['od_send_cost2']); ?></strong>원
        </p>
        <?php if ($default['de_hope_date_use']) { ?><p>희망배송일은 <?=$od['od_hope_date']; ?> (<?=get_yoil($od['od_hope_date']); ?>) 입니다.</p><?php } ?>
        <?php if($od['od_mobile']) { ?>
        <p>모바일 쇼핑몰의 주문입니다.</p>
        <?php } ?>
    </div>
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        for($i=0; $row=sql_fetch_array($result); $i++) {
            ...
            <tr>
                <?php if($k == 0) { ?>
                <td rowspan="<?=$rowspan; ?>" class="td_left">
                    <a href="./itemform.php?w=u&amp;it_id=<?=$row['it_id']; ?>"><?=$image; ?> <?=stripslashes($row['it_name']); ?></a>
                    <?php if($od['od_tax_flag'] && $row['ct_notax']) echo '[비과세상품]'; ?>
                </td>
                <td rowspan="<?=$rowspan; ?>" class="td_chk">
                    <label for="sit_sel_<?=$i; ?>" class="sound_only"><?=$row['it_name']; ?> 옵션 전체선택</label>
                    <input type="checkbox" id="sit_sel_<?=$i; ?>" name="it_sel[]">
                </td>
                <?php } ?>
                <td class="td_left">
                    <label for="ct_chk_<?=$chk_cnt; ?>" class="sound_only"><?=get_text($opt['ct_option']); ?></label>
                    <input type="checkbox" name="ct_chk[<?=$chk_cnt; ?>]" id="ct_chk_<?=$chk_cnt; ?>" value="<?=$chk_cnt; ?>" class="sct_sel_<?=$i; ?>">
                    <input type="hidden" name="ct_id[<?=$chk_cnt; ?>]" value="<?=$opt['ct_id']; ?>">
                    <?=get_text($opt['ct_option']); ?>
                </td>
                <td class="td_mngsmall"><?=$opt['ct_status']; ?></td>
                <td class="td_num">
                    <label for="ct_qty_<?=$chk_cnt; ?>" class="sound_only"><?=get_text($opt['ct_option']); ?> 수량</label>
                    <input type="text" name="ct_qty[<?=$chk_cnt; ?>]" id="ct_qty_<?=$chk_cnt; ?>" value="<?=$opt['ct_qty']; ?>" required class="frm_input required" size="5">
                </td>
                <td class="td_num_right "><?=number_format($opt_price); ?></td>
                <td class="td_num_right"><?=number_format($ct_price['stotal']); ?></td>
                <td class="td_num_right"><?=number_format($opt['cp_price']); ?></td>
                <td class=" td_num_right"><?=number_format($ct_point['stotal']); ?></td>
    ...
    <section id="sodr_qty_log">
        ...
        <div>
            <?=conv_content($od['od_mod_history'], 0); ?>
        </div>
    </section>
    <?php } ?>
...
<section id="anc_sodr_pay">
    ...
    <div class="tbl_head01 tbl_wrap">
        <strong class="sodr_nonpay">미수금 <?=display_price($od['od_misu']); ?></strong>
        ...
        <tr>
            ...
            <td class="td_numbig"><?=display_price($od['od_send_cost'] + $od['od_send_cost2']); ?></td>
            <td class="td_numbig"><?=display_point($od['od_receipt_point']); ?></td>
...
<section class="">
    ...
    <div class="compare_wrap">
        ...
        <section id="anc_sodr_chk" class="compare_left">
            ...
            <div class="tbl_frm01">
                ...
                <tr>
                    ...
                    <td><?=display_price($od['od_receipt_price']); ?></td>
                </tr>
                <tr>
                    <th scope="row">입금자</th>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <?php } else { ?><?=$od['od_receipt_time']; ?> (<?=get_yoil($od['od_receipt_time']); ?>)
                        <?php } ?>
                    </td>
                </tr>
                <?php } ?>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <?php } else { ?><?=display_price($od['od_receipt_price']); ?>
                        <?php } ?>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        ...
                        <?php } else { ?><?=substr($od['od_receipt_time'], 0, 20); ?>
                        <?php } ?>
                    </td>
                </tr>
                <?php } ?>
                ...
                <tr>
                    <th scope="row" class="sodr_sppay"><?=$s_receipt_way; ?> 결제금액</th>
                    <td>
                        <?php if ($od['od_receipt_time'] == "0000-00-00 00:00:00") {?>0원
                        <?php } else { ?><?=display_price($od['od_receipt_price']); ?>
                        <?php } ?>
                ...
                <tr>
                    <th scope="row" class="sodr_sppay"><?=$s_receipt_way; ?> 승인일시</th>
                    <td>
                ...
                <tr>
                    ...
                    <td><?=display_price($od['od_tax_mny']); ?></td>
                </tr>
                <tr>
                    <th scope="row">과세부가세액</th>
                    <td><?=display_price($od['od_vat_mny']); ?></td>
                </tr>
                <tr>
                    <th scope="row">비과세공급가액</th>
                    <td><?=display_price($od['od_free_mny']); ?></td>
                </tr>
                <?php } ?>
                <tr>
                    <th scope="row">주문금액할인</th>
                    <td><?=display_price($od['od_coupon']); ?></td>
                </tr>
                <tr>
                    <th scope="row">포인트</th>
                ...
                <tr>
                    ...
                    <td><?=is_null_time($od['od_invoice_time']) ? "" : $od['od_invoice_time']; ?></td>
                </tr>
                <?php } ?>
                <tr>
                ...
                <tr>
                    ...
                    <td><?=display_price($od['od_send_coupon']); ?></td>
                </tr>
                <?php } ?>
                <tr>
                    <th scope="row"><label for="od_send_cost2">추가배송비</label></th>
                    <td>
                        <input type="text" name="od_send_cost2" value="<?=$od['od_send_cost2']; ?>" id="od_send_cost2" class="frm_input" size="10"> 원
                    </td>
                </tr>
                <?php
                ...
                <tr>
                    ...
                    <?php } else { ?>
                        <a href="javascript:;" onclick="window.open('<?=G5_SHOP_URL; ?>/taxsave.php?od_id=<?=$od_id; ?>', 'taxsave', 'width=550,height=400,scrollbars=1,menus=0');">현금영수증 발급</a>
                    <?php } ?>
                    </td>
                </tr>
                <?php
        ...
        <section id="anc_sodr_paymo" class="compare_right">
            ...
            <div class="tbl_frm01">
                ...
                <tr>
                    <th scope="row"><label for="od_receipt_price"><?=$od['od_settle_case']; ?> 입금액</label></th>
                    <td>
                        <?=$html_receipt_chk; ?>
                        <input type="text" name="od_receipt_price" value="<?=$od['od_receipt_price']; ?>" id="od_receipt_price" class="frm_input"> 원
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        ...
                        <br>
                        <?php } ?>
                        <input type="text" name="od_deposit_name" value="<?=get_text($od['od_deposit_name']); ?>" id="od_deposit_name" class="frm_input">
                    </td>
                </tr>
                <tr>
                    <th scope="row"><label for="od_receipt_time">입금 확인일시</label></th>
                    <td>
                        <input type="checkbox" name="od_bank_chk" id="od_bank_chk" value="<?=date("Y-m-d H:i:s", G5_SERVER_TIME); ?>" onclick="if (this.checked == true) this.form.od_receipt_time.value=this.form.od_bank_chk.value; else this.form.od_receipt_time.value = this.form.od_receipt_time.defaultValue;">
                        <label for="od_bank_chk">현재 시간으로 설정</label><br>
                        <input type="text" name="od_receipt_time" value="<?=is_null_time($od['od_receipt_time']) ? "" : $od['od_receipt_time']; ?>" id="od_receipt_time" class="frm_input" maxlength="19">
                    </td>
                </tr>
                <?php } ?>
                ...
                <tr>
                    <th scope="row"><label for="od_receipt_price"><?=$od['od_settle_case']; ?> 결제액</label></th>
                    <td>
                        <?=$html_receipt_chk; ?>
                        <input type="text" name="od_receipt_price" value="<?=$od['od_receipt_price']; ?>" id="od_receipt_price" class="frm_input"> 원
                    </td>
                ...
                <tr>
                    ...
                    <td>
                        <input type="checkbox" name="od_hp_chk" id="od_hp_chk" value="<?=date("Y-m-d H:i:s", G5_SERVER_TIME); ?>" onclick="if (this.checked == true) this.form.od_receipt_time.value=this.form.od_hp_chk.value; else this.form.od_receipt_time.value = this.form.od_receipt_time.defaultValue;">
                        <label for="od_hp_chk">현재 시간으로 설정</label><br>
                        <input type="text" name="od_receipt_time" value="<?=is_null_time($od['od_receipt_time']) ? "" : $od['od_receipt_time']; ?>" id="op_receipt_time" class="frm_input" size="19" maxlength="19">
                    </td>
                </tr>
                <?php } ?>
                ...
                <tr>
                    ...
                    <td>
                        <?=$html_receipt_chk; ?>
                        <input type="text" name="od_receipt_price" id="od_receipt_price" value="<?=$od['od_receipt_price']; ?>" class="frm_input" size="10"> 원
                    </td>
                </tr>
                <tr>
                    <th scope="row" class="sodr_sppay"><label for="od_receipt_time">카드 승인일시</label></th>
                    <td>
                        <input type="checkbox" name="od_card_chk" id="od_card_chk" value="<?=date("Y-m-d H:i:s", G5_SERVER_TIME); ?>" onclick="if (this.checked == true) this.form.od_receipt_time.value=this.form.od_card_chk.value; else this.form.od_receipt_time.value = this.form.od_receipt_time.defaultValue;">
                        <label for="od_card_chk">현재 시간으로 설정</label><br>
                        <input type="text" name="od_receipt_time" value="<?=is_null_time($od['od_receipt_time']) ? "" : $od['od_receipt_time']; ?>" id="od_receipt_time" class="frm_input" size="19" maxlength="19">
                    </td>
                </tr>
                <?php } ?>
                ...
                <?php if ($od['od_settle_case'] == '간편결제' || ($od['od_pg'] == 'inicis' && is_inicis_order_pay($od['od_settle_case']) )) { ?>
                <tr>
                    <th scope="row" class="sodr_sppay"><label for="od_receipt_price"><?=$s_receipt_way; ?> 결제금액</label></th>
                    <td>
                        <?=$html_receipt_chk; ?>
                        <input type="text" name="od_receipt_price" id="od_receipt_price" value="<?=$od['od_receipt_price']; ?>" class="frm_input" size="10"> 원
                    </td>
                ...
                <tr>
                    <th scope="row" class="sodr_sppay"><label for="od_receipt_time"><?=$s_receipt_way; ?> 승인일시</label></th>
                    <td>
                        <input type="checkbox" name="od_card_chk" id="od_card_chk" value="<?=date("Y-m-d H:i:s", G5_SERVER_TIME); ?>" onclick="if (this.checked == true) this.form.od_receipt_time.value=this.form.od_card_chk.value; else this.form.od_receipt_time.value = this.form.od_receipt_time.defaultValue;">
                        <label for="od_card_chk">현재 시간으로 설정</label><br>
                        <input type="text" name="od_receipt_time" value="<?=is_null_time($od['od_receipt_time']) ? "" : $od['od_receipt_time']; ?>" id="od_receipt_time" class="frm_input" size="19" maxlength="19">
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" name="od_refund_price" value="<?=$od['od_refund_price']; ?>" id="od_refund_price" class="frm_input" size="10"> 원
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td>
                        <input type="checkbox" id="od_delivery_chk" value="<?=$default['de_delivery_company']; ?>" onclick="chk_delivery_company()">
                        <label for="od_delivery_chk">기본 배송회사로 설정</label><br>
                        <input type="text" name="od_delivery_company" id="od_delivery_company" value="<?=$od['od_delivery_company']; ?>" class="frm_input">
                    </td>
                </tr>
                <tr>
                    <th scope="row"><label for="od_invoice_time">배송일시</label></th>
                    <td>
                        <input type="checkbox" id="od_invoice_chk" value="<?=date("Y-m-d H:i:s", G5_SERVER_TIME); ?>" onclick="chk_invoice_time()">
                        <label for="od_invoice_chk">현재 시간으로 설정</label><br>
                        <input type="text" name="od_invoice_time" id="od_invoice_time" value="<?=is_null_time($od['od_invoice_time']) ? "" : $od['od_invoice_time']; ?>" class="frm_input" maxlength="19">
                    </td>
                </tr>
    ...
    <div class="btn_confirm01 btn_confirm">
        ...
        <a href="./personalpayform.php?popup=yes&amp;od_id=<?=$od_id; ?>" id="personalpay_add" class="btn btn_02">개인결제추가</a>
        <?php } ?>
        ...
        <a href="./orderpartcancel.php?od_id=<?=$od_id; ?>" id="orderpartcancel" class="btn btn_02"><?=$od['od_settle_case']; ?> 부분취소</a>
        <?php } ?>
        <a href="./orderlist.php?<?=$qstr; ?>" class="btn btn_02">목록</a>
    </div>
    </form>
</section>
...
<section id="anc_sodr_memo">
    ...
    <div class="tbl_wrap">
        ...
        <textarea name="od_shop_memo" id="od_shop_memo" rows="8"><?=html_purifier(stripslashes($od['od_shop_memo'])); ?></textarea>
    </div>
...
<section>
        ...
        <section id="anc_sodr_orderer" class="compare_left">
            ...
            <div class="tbl_frm01">
                ...
                <tr>
                    ...
                    <td><input type="text" name="od_name" value="<?=get_text($od['od_name']); ?>" id="od_name" required class="frm_input required"></td>
                </tr>
                <tr>
                    <th scope="row"><label for="od_tel"><span class="sound_only">주문하신 분 </span>전화번호</label></th>
                    <td><input type="text" name="od_tel" value="<?=get_text($od['od_tel']); ?>" id="od_tel" required class="frm_input required"></td>
                </tr>
                <tr>
                    <th scope="row"><label for="od_hp"><span class="sound_only">주문하신 분 </span>핸드폰</label></th>
                    <td><input type="text" name="od_hp" value="<?=get_text($od['od_hp']); ?>" id="od_hp" class="frm_input"></td>
                </tr>
                <tr>
                    <th scope="row"><span class="sound_only">주문하시는 분 </span>주소</th>
                    <td>
                        <label for="od_zip" class="sound_only">우편번호</label>
                        <input type="text" name="od_zip" value="<?=get_text($od['od_zip1']).get_text($od['od_zip2']); ?>" id="od_zip" required class="frm_input required" size="5">
                        ...
                        <input type="text" name="od_addr1" value="<?=get_text($od['od_addr1']); ?>" id="od_addr1" required class="frm_input required" size="35">
                        <label for="od_addr1">기본주소</label><br>
                        <input type="text" name="od_addr2" value="<?=get_text($od['od_addr2']); ?>" id="od_addr2" class="frm_input" size="35">
                        <label for="od_addr2">상세주소</label>
                        <br>
                        <input type="text" name="od_addr3" value="<?=get_text($od['od_addr3']); ?>" id="od_addr3" class="frm_input" size="35">
                        <label for="od_addr3">참고항목</label>
                        <input type="hidden" name="od_addr_jibeon" value="<?=get_text($od['od_addr_jibeon']); ?>"><br>
                    </td>
                </tr>
                <tr>
                    <th scope="row"><label for="od_email"><span class="sound_only">주문하신 분 </span>E-mail</label></th>
                    <td><input type="text" name="od_email" value="<?=$od['od_email']; ?>" id="od_email" required class="frm_input required" size="30"></td>
                </tr>
                <tr>
        ...
        <section id="anc_sodr_taker" class="compare_right">
            ...
            <div class="tbl_frm01">
                ...
                <tr>
                    ...
                    <td><input type="text" name="od_b_name" value="<?=get_text($od['od_b_name']); ?>" id="od_b_name" required class="frm_input required"></td>
                </tr>
                <tr>
                    <th scope="row"><label for="od_b_tel"><span class="sound_only">받으시는 분 </span>전화번호</label></th>
                    <td><input type="text" name="od_b_tel" value="<?=get_text($od['od_b_tel']); ?>" id="od_b_tel" required class="frm_input required"></td>
                </tr>
                <tr>
                    <th scope="row"><label for="od_b_hp"><span class="sound_only">받으시는 분 </span>핸드폰</label></th>
                    <td><input type="text" name="od_b_hp" value="<?=get_text($od['od_b_hp']); ?>" id="od_b_hp" class="frm_input required"></td>
                </tr>
                <tr>
                    <th scope="row"><span class="sound_only">받으시는 분 </span>주소</th>
                    <td>
                        <label for="od_b_zip" class="sound_only">우편번호</label>
                        <input type="text" name="od_b_zip" value="<?=get_text($od['od_b_zip1']).get_text($od['od_b_zip2']); ?>" id="od_b_zip" required class="frm_input required" size="5">
                        ...
                        <input type="text" name="od_b_addr1" value="<?=get_text($od['od_b_addr1']); ?>" id="od_b_addr1" required class="frm_input required" size="35">
                        <label for="od_b_addr1">기본주소</label>
                        <input type="text" name="od_b_addr2" value="<?=get_text($od['od_b_addr2']); ?>" id="od_b_addr2" class="frm_input" size="35">
                        <label for="od_b_addr2">상세주소</label>
                        <input type="text" name="od_b_addr3" value="<?=get_text($od['od_b_addr3']); ?>" id="od_b_addr3" class="frm_input" size="35">
                        <label for="od_b_addr3">참고항목</label>
                        <input type="hidden" name="od_b_addr_jibeon" value="<?=get_text($od['od_b_addr_jibeon']); ?>"><br>
                    </td>
                </tr>
    ...
    <div class="btn_confirm01 btn_confirm">
        ...
        <a href="./orderlist.php?<?=$qstr; ?>" class="btn">목록</a>
    </div>
