...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr>
        <th scope="row"><label for="ii_article_<?=$el_name; ?>"><?=$el_title; ?></label></th>
        <td>
            <input type="hidden" name="ii_article[]" value="<?=$el_name; ?>">
            <?php if ($el_example != "") echo help($el_example); ?>
            <input type="text" name="ii_value[]" value="<?=get_text($el_value); ?>" id="ii_article_<?=$el_name; ?>" required class="frm_input required" />
        </td>
        <?php if ($el_no == 0) { ?>
        <td rowspan="<?=$el_length; ?>" class="td_grpset">
