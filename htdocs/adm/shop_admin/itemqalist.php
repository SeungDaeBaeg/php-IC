...
<div class="local_ov01 local_ov">
    <?=$listall; ?>
    <span class="btn_ov01"><span class="ov_txt"> 전체 문의내역</span><span class="ov_num"> <?=$total_count; ?>건</span></span>
</div>
...
<div class="tbl_head01 tbl_wrap" id="itemqalist">
    ...
    <tr class="<?=$bg; ?>">
        <td class="td_chk">
            <label for="chk_<?=$i; ?>" class="sound_only"><?=get_text($row['iq_subject']) ?> 상품문의</label>
            <input type="checkbox" name="chk[]" value="<?=$i ?>" id="chk_<?=$i; ?>">
            <input type="hidden" name="iq_id[<?=$i; ?>]" value="<?=$row['iq_id']; ?>">
        </td>
        <td class="td_left"><a href="<?=$href; ?>"><?=get_it_image($row['it_id'], 50, 50); ?> <?=cut_str($row['it_name'],30); ?></a></td>
        <td class="td_left">
            <a href="#" class="qa_href" onclick="return false;" target="<?=$i; ?>"><?=get_text($row['iq_subject']); ?> <span class="tit_op">열기</span></a>
            <div id="qa_div<?=$i; ?>" class="qa_div" style="display:none;">
                <div class="qa_q">
                    <strong>문의내용</strong>
        ...
        <td class="td_boolean"><?=$answer; ?></td>
        <td class="td_mng td_mng_s">
            <a href="./itemqaform.php?w=u&amp;iq_id=<?=$row['iq_id']; ?>&amp;<?=$qstr; ?>" class="btn btn_03"><span class="sound_only"><?=get_text($row['iq_subject']); ?> </span>수정</a>
        </td>
    </tr>
    <?php
    }
