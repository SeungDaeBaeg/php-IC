...
<div class="local_ov01 local_ov">
    <span class="btn_ov01"><span class="ov_txt"> <?=($sql_search) ? '검색' : '등록'; ?>된 배너 </span><span class="ov_num"> <?=$total_count; ?>개</span></span>
    ...
    <select name="bn_position" id="bn_position">
        <option value=""<?=get_selected($bn_position, '', true); ?>>위치 전체</option>
        <option value="메인"<?=get_selected($bn_position, '메인', true); ?>>메인</option>
        <option value="왼쪽"<?=get_selected($bn_position, '왼쪽', true); ?>>왼쪽</option>
    </select>

    <select name="bn_device" id="bn_device">
        <option value="both"<?=get_selected($bn_device, 'both', true); ?>>PC와 모바일</option>
        <option value="pc"<?=get_selected($bn_device, 'pc'); ?>>PC</option>
        <option value="mobile"<?=get_selected($bn_device, 'mobile'); ?>>모바일</option>
    </select>

    <select name="bn_time" id="bn_time">
        <option value=""<?=get_selected($bn_time, '', true); ?>>배너 시간 전체</option>
        <option value="ing"<?=get_selected($bn_time, 'ing'); ?>>진행중인 배너</option>
        <option value="end"<?=get_selected($bn_time, 'end'); ?>>종료된 배너</option>
    </select>
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr class="<?=$bg; ?>">
        <td headers="th_id" rowspan="2" class="td_num"><?=$row['bn_id']; ?></td>
        <td headers="th_dvc"><?=$bn_device; ?></td>
        <td headers="th_loc"><?=$row['bn_position']; ?></td>
        <td headers="th_st" class="td_datetime"><?=$bn_begin_time; ?></td>
        <td headers="th_end" class="td_datetime"><?=$bn_end_time; ?></td>
        <td headers="th_odr" class="td_num"><?=$row['bn_order']; ?></td>
        <td headers="th_hit" class="td_num"><?=$row['bn_hit']; ?></td>
        <td headers="th_mng" class="td_mng td_mns_m">
            <a href="./bannerform.php?w=u&amp;bn_id=<?=$row['bn_id']; ?>" class="btn btn_03">수정</a>
            <a href="./bannerformupdate.php?w=d&amp;bn_id=<?=$row['bn_id']; ?>" onclick="return delete_confirm(this);" class="btn btn_02">삭제</a>
        </td>
    </tr>
    <tr class="<?=$bg; ?>">
        <td headers="th_img" colspan="7" class="td_img_view sbn_img">
            <div class="sbn_image"><?=$bn_img; ?></div>
