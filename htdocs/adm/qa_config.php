...
<section id="anc_cf_qa_config">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            ...
            <td>
                <input type="text" name="qa_title" value="<?=get_sanitize_input($qaconfig['qa_title']); ?>" id="qa_title" required class="required frm_input" size="40">
                <a href="<?=G5_BBS_URL; ?>/qalist.php" class="btn_frmline">1:1문의 바로가기</a>
            </td>
        </tr>
        <tr>
            ...
            <td>
                ...
                <input type="text" name="qa_category" value="<?=get_sanitize_input($qaconfig['qa_category']); ?>" id="qa_category" required class="required frm_input" size="70">
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="qa_skin">스킨 디렉토리<strong class="sound_only">필수</strong></label></th>
            <td>
                <?=get_skin_select('qa', 'qa_skin', 'qa_skin', $qaconfig['qa_skin'], 'required'); ?>
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="qa_mobile_skin">모바일 스킨 디렉토리<strong class="sound_only">필수</strong></label></th>
            <td>
                <?=get_mobile_skin_select('qa', 'qa_mobile_skin', 'qa_mobile_skin', $qaconfig['qa_mobile_skin'], 'required'); ?>
            </td>
        </tr>
        <tr>
            <th scope="row">이메일 입력</th>
            <td>
                <input type="checkbox" name="qa_use_email" value="1" id="qa_use_email" <?=$qaconfig['qa_use_email']?'checked':''; ?>> <label for="qa_use_email">보이기</label>
                <input type="checkbox" name="qa_req_email" value="1" id="qa_req_email" <?=$qaconfig['qa_req_email']?'checked':''; ?>> <label for="qa_req_email">필수입력</label>
            </td>
        </tr>
        <tr>
            <th scope="row">휴대폰 입력</th>
            <td>
                <input type="checkbox" name="qa_use_hp" value="1" id="qa_use_hp" <?=$qaconfig['qa_use_hp']?'checked':''; ?>> <label for="qa_use_hp">보이기</label>
                <input type="checkbox" name="qa_req_hp" value="1" id="qa_req_hp" <?=$qaconfig['qa_req_hp']?'checked':''; ?>> <label for="qa_req_hp">필수입력</label>
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="qa_use_sms">SMS 알림</label></th>
            <td>
                <?=help('휴대폰 입력을 사용하실 경우 문의글 등록시 등록자가 답변등록시 SMS 알림 수신을 선택할 수 있도록 합니다.<br>SMS 알림을 사용하기 위해서는 기본환경설정 > <a href="'.G5_ADMIN_URL.'/config_form.php#anc_cf_sms">SMS 설정</a>을 하셔야 합니다.') ?>
        ...
        <tr>
            ...
            <td>
                <?=help('SMS 알림 전송시 발신번호로 사용됩니다.'); ?>
                <input type="text" name="qa_send_number" value="<?=get_sanitize_input($qaconfig['qa_send_number']); ?>" id="qa_send_number" class="frm_input"  size="30">
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="qa_admin_hp">관리자 휴대폰번호</label></th>
            <td>
                <?=help('관리자 휴대폰번호를 입력하시면 문의글 등록시 등록하신 번호로 SMS 알림이 전송됩니다.<br>SMS 알림을 사용하지 않으시면 알림이 전송되지 않습니다.'); ?>
                <input type="text" name="qa_admin_hp" value="<?=get_sanitize_input($qaconfig['qa_admin_hp']); ?>" id="qa_admin_hp" class="frm_input"  size="30">
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="qa_admin_email">관리자 이메일</label></th>
            <td>
                <?=help('관리자 이메일을 입력하시면 문의글 등록시 등록하신 이메일로 알림이 전송됩니다.'); ?>
                <input type="text" name="qa_admin_email" value="<?=get_sanitize_input($qaconfig['qa_admin_email']); ?>" id="qa_admin_email" class="frm_input"  size="50">
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>
                <?=help('목록에서의 제목 글자수') ?>
                <input type="text" name="qa_subject_len" value="<?=$qaconfig['qa_subject_len'] ?>" id="qa_subject_len" required class="required numeric frm_input"  size="4">
            </td>
        </tr>
        <tr>
            ...
            <td>
                <?=help('목록에서의 제목 글자수') ?>
                <input type="text" name="qa_mobile_subject_len" value="<?=$qaconfig['qa_mobile_subject_len'] ?>" id="qa_mobile_subject_len" required class="required numeric frm_input"  size="4">
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="qa_page_rows">페이지당 목록 수<strong class="sound_only"> 필수</strong></label></th>
            <td>
                <input type="text" name="qa_page_rows" value="<?=$qaconfig['qa_page_rows'] ?>" id="qa_page_rows" required class="required numeric frm_input"  size="4">
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="qa_mobile_page_rows">모바일 페이지당 목록 수<strong class="sound_only"> 필수</strong></label></th>
            <td>
                <input type="text" name="qa_mobile_page_rows" value="<?=$qaconfig['qa_mobile_page_rows'] ?>" id="qa_mobile_page_rows" required class="required numeric frm_input"  size="4">
            </td>
        </tr>
        <tr>
            ...
            <td>
                ...
                <input type="text" name="qa_image_width" value="<?=$qaconfig['qa_image_width'] ?>" id="qa_image_width" required class="required numeric frm_input"  size="4"> 픽셀
            </td>
        </tr>
        <tr>
            ...
            <td>
                ...
                업로드 파일 한개당 <input type="text" name="qa_upload_size" value="<?=$qaconfig['qa_upload_size'] ?>" id="qa_upload_size" required class="required numeric frm_input"  size="10"> bytes 이하
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="qa_include_head">상단 파일 경로</label></th>
            <td>
                <input type="text" name="qa_include_head" value="<?=$qaconfig['qa_include_head'] ?>" id="qa_include_head" class="frm_input" size="50">
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="qa_include_tail">하단 파일 경로</label></th>
            <td>
                <input type="text" name="qa_include_tail" value="<?=$qaconfig['qa_include_tail'] ?>" id="qa_include_tail" class="frm_input" size="50">
            </td>
        </tr>
        ...
        <tr>
            ...
            <td>
                <?=editor_html("qa_content_head", get_text(html_purifier($qaconfig['qa_content_head']), 0)); ?>
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="qa_content_tail">하단 내용</label></th>
            <td>
                <?=editor_html("qa_content_tail", get_text(html_purifier($qaconfig['qa_content_tail']), 0)); ?>
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="qa_mobile_content_head">모바일 상단 내용</label></th>
            <td>
                <?=editor_html("qa_mobile_content_head", get_text(html_purifier($qaconfig['qa_mobile_content_head']), 0)); ?>
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="qa_mobile_content_tail">모바일 하단 내용</label></th>
            <td>
                <?=editor_html("qa_mobile_content_tail", get_text(html_purifier($qaconfig['qa_mobile_content_tail']), 0)); ?>
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="qa_insert_content">글쓰기 기본 내용</label></th>
            <td>
                <textarea id="qa_insert_content" name="qa_insert_content" rows="5"><?=html_purifier($qaconfig['qa_insert_content']); ?></textarea>
            </td>
        </tr>
        <?php for ($i=1; $i<=5; $i++) { ?>
        <tr>
            ...
            <td class="td_extra">
                ...
                <input type="text" name="qa_<?=$i ?>_subj" id="qa_<?=$i ?>_subj" value="<?=get_text($qaconfig['qa_'.$i.'_subj']) ?>" class="frm_input">
                <label for="qa_<?=$i ?>">여분필드 <?=$i ?> 값</label>
                <input type="text" name="qa_<?=$i ?>" value="<?=get_text($qaconfig['qa_'.$i]) ?>" id="qa_<?=$i ?>" class="frm_input">
            </td>
        </tr>
        <?php } ?>
        </tbody>
...
function frm_check_file(){
    var qa_include_head = "<?=$qaconfig['qa_include_head']; ?>";
    var qa_include_tail = "<?=$qaconfig['qa_include_tail']; ?>";
...
function fqaconfigform_submit(f)
{
    ...
    if( captcha_chk ) {
        <?=isset($captcha_js) ? $captcha_js : ''; // 캡챠 사용시 자바스크립트에서 입력된 캡챠를 검사함  ?>
    }
