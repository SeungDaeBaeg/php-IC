...
<select name="sfl" id="sfl">
    ...
    <option value="po_content"<?=get_selected($sfl, "po_content"); ?>>내용</option>
</select>
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr class="<?=$bg; ?>">
        ...
        <td class="td_left"><a href="?sfl=mb_id&amp;stx=<?=$row['mb_id'] ?>"><?=$row['mb_id'] ?></a></td>
        <td class="td_left"><?=get_text($row2['mb_name']); ?></td>
        <td class="td_left sv_use"><div><?=$mb_nick ?></div></td>
        <td class="td_left"><?=$link1 ?><?=$row['po_content'] ?><?=$link2 ?></td>
        <td class="td_num td_pt"><?=number_format($row['po_point']) ?></td>
        <td class="td_datetime"><?=$row['po_datetime'] ?></td>
        <td class="td_datetime2<?=$expr; ?>">
            <?php if ($row['po_expired'] == 1) { ?>
            만료<?=substr(str_replace('-', '', $row['po_expire_date']), 2); ?>
            <?php } else echo $row['po_expire_date'] == '9999-12-31' ? '&nbsp;' : $row['po_expire_date']; ?>
        </td>
        <td class="td_num td_pt"><?=number_format($row['po_mb_point']) ?></td>
    </tr>
