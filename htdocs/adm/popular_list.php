...
<div class="sch_last">
    ...
    <select name="sfl" id="sfl">
        <option value="pp_word"<?=get_selected($sfl, "pp_word"); ?>>검색어</option>
        <option value="pp_date"<?=get_selected($sfl, "pp_date"); ?>>등록일</option>
    </select>
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr class="<?=$bg; ?>">
        <td class="td_chk">
            <label for="chk_<?=$i; ?>" class="sound_only"><?=$word ?></label>
            <input type="checkbox" name="chk[]" value="<?=$row['pp_id'] ?>" id="chk_<?=$i ?>">
        </td>
        <td class="td_left"><a href="<?=$_SERVER['SCRIPT_NAME'] ?>?sfl=pp_word&amp;stx=<?=$word ?>"><?=$word ?></a></td>
        <td><?=$row['pp_date'] ?></td>
        <td><?=$row['pp_ip'] ?></td>
    </tr>
