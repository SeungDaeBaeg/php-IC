...
<div class="local_ov01 local_ov">
    <?=$listall ?>
    <span class="btn_ov01"><span class="ov_txt">전체그룹</span><span class="ov_num">  <?=number_format($total_count) ?>개</span></span>
</div>

<form name="fsearch" id="fsearch" class="local_sch01 local_sch" method="get">
<label for="sfl" class="sound_only">검색대상</label>
<select name="sfl" id="sfl">
    <option value="gr_subject"<?=get_selected($sfl, "gr_subject"); ?>>제목</option>
    <option value="gr_id"<?=get_selected($sfl, "gr_id"); ?>>ID</option>
    <option value="gr_admin"<?=get_selected($sfl, "gr_admin"); ?>>그룹관리자</option>
</select>
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr class="<?=$bg; ?>">
        <td class="td_chk">
            ...
            <label for="chk_<?=$i; ?>" class="sound_only"><?=get_text($row['gr_subject']); ?> 그룹</label>
            <input type="checkbox" name="chk[]" value="<?=$i ?>" id="chk_<?=$i ?>">
        </td>
        <td class="td_left"><a href="<?=G5_BBS_URL ?>/group.php?gr_id=<?=$row['gr_id'] ?>"><?=$row['gr_id'] ?></a></td>
        <td class="td_input">
            <label for="gr_subject_<?=$i; ?>" class="sound_only">그룹제목</label>
            <input type="text" name="gr_subject[<?=$i ?>]" value="<?=get_text($row['gr_subject']) ?>" id="gr_subject_<?=$i ?>" class="tbl_input">
        </td>
        <td class="td_mng td_input">
        <?php if ($is_admin == 'super'){ ?>
            <label for="gr_admin_<?=$i; ?>" class="sound_only">그룹관리자</label>
            <input type="text" name="gr_admin[<?=$i ?>]" value="<?=get_sanitize_input($row['gr_admin']); ?>" id="gr_admin_<?=$i ?>" class="tbl_input" size="10" maxlength="20">
        <?php }else{ ?>
            <input type="hidden" name="gr_admin[<?=$i ?>]" value="<?=get_sanitize_input($row['gr_admin']); ?>"><?=get_text($row['gr_admin']); ?>
        <?php } ?>
        </td>
        <td class="td_num"><a href="./board_list.php?sfl=a.gr_id&amp;stx=<?=$row['gr_id'] ?>"><?=$row2['cnt'] ?></a></td>
        <td class="td_numsmall">
             <label for="gr_use_access_<?=$i; ?>" class="sound_only">접근회원 사용</label>
            <input type="checkbox" name="gr_use_access[<?=$i ?>]" <?=$row['gr_use_access']?'checked':'' ?> value="1" id="gr_use_access_<?=$i ?>">
        </td>
        <td class="td_num"><a href="./boardgroupmember_list.php?gr_id=<?=$row['gr_id'] ?>"><?=$row1['cnt'] ?></a></td>
        <td class="td_numsmall">
            <label for="gr_order_<?=$i; ?>" class="sound_only">메인메뉴 출력순서</label>
            <input type="text" name="gr_order[<?=$i ?>]" value="<?=$row['gr_order'] ?>" id="gr_order_<?=$i ?>" class="tbl_input" size="2">
        </td>
        <td class="td_mng">
            <label for="gr_device_<?=$i; ?>" class="sound_only">접속기기</label>
            <select name="gr_device[<?=$i ?>]" id="gr_device_<?=$i ?>">
        ...
        <td class="td_mng td_mng_s"><?=$s_upd ?></td>
    </tr>
