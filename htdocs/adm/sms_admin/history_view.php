...
<input type="hidden" name="wr_no" value="<?=get_sanitize_input($wr_no); ?>">
<input type="hidden" name="wr_renum" value="<?=get_sanitize_input($wr_renum); ?>">
<input type="hidden" name="page" value="<?=get_sanitize_input($page); ?>">
<input type="hidden" name="st" value="<?=get_sanitize_input($st); ?>">
<input type="hidden" name="sv" value="<?=get_sanitize_input($sv); ?>">
<label for="sst" class="sound_only">검색대상</label>
<select name="sst" id="sst">
    <option value="hs_name" <?=get_selected('hs_name', $sst); ?>>이름</option>
    <option value="hs_hp" <?=get_selected('hs_hp', $sst); ?>>휴대폰번호</option>
</select>
<label for="ssv" class="sound_only">검색어<strong class="sound_only"> 필수</strong></label>
<input type="text" name="ssv" value="<?=get_sanitize_input($ssv); ?>" id="ssv" class="frm_input">
<input type="submit" value="검색" class="btn_submit">
</form>

<div id="sms5_sent">
    <div class="local_ov01 local_ov">
        <span class="btn_ov01"><span class="ov_txt">전송건수</span><span class="ov_num"> <?=number_format($write['wr_total'])?> 건</span></span>
        <span class="btn_ov01"><span class="ov_txt">성공건수 </span><span class="ov_num"><?=number_format($write['wr_success'])?> 건</span></span>
        <span class="btn_ov01"><span class="ov_txt">실패건수 </span><span class="ov_num"><?=number_format($write['wr_failure'])?> 건</span></span>
        <span class="btn_ov01"><span class="ov_txt">전송일시 </span><span class="ov_num"><?=$write['wr_datetime']?></span></span>
        <span class="btn_ov01"><span class="ov_txt">예약일시 </span><span class="ov_num"><?=$write['wr_booking']?></span></span>
        <span class="btn_ov01"><span class="ov_txt">회신번호 </span><span class="ov_num"><?=$write['wr_reply']?></span></span>
    </div>
    ...
    <div id="con_sms" class="sms5_box">
        ...
        <textarea class="box_txt is_overview" readonly><?=$write['wr_message'];?></textarea>
    </div>
    ...
    <tr>
        ...
        <td class="td_mng">
            <a href="./history_view.php?page=<?=$page?>&amp;st=<?=$st?>&amp;sv=<?=$sv?>&amp;wr_no=<?=$res['wr_no']?>&amp;wr_renum=<?=$res['wr_renum']?>" class="btn btn_03">수정</a>
            <!-- <a href="./history_del.php?page=<?=$page?>&amp;st=<?=$st?>&amp;sv=<?=$sv?>&amp;wr_no=<?=$res['wr_no']?>&amp;wr_renum=<?=$res['wr_renum']?>">삭제</a> -->
        </td>
    </tr>
    <?php } ?>
    </tbody>
    ...
    <h2>문자전송 목록 <?=$re_text?></h2>

    <div class="btn_add01 btn_add">
        <a href="javascript:all_send()">전체 재전송</a>
        <a href="javascript:re_send()">실패내역 재전송</a>
        <?php if (!$wr_renum) {?>
        <a href="./history_list.php?page=<?=$page?>&amp;st=<?=$st?>&amp;sv=<?=$sv?>">목록</a>
        <?php } else { ?>
        <a href="./history_view.php?page=<?=$page?>&amp;st=<?=$st?>&amp;sv=<?=$sv?>&amp;wr_no=<?=$wr_no?>">뒤로가기</a>
        <?php } ?>
    </div>
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        ?>
        <tr class="<?=$bg; ?>">
            <td class="td_numsmall"><?=number_format($vnum--)?></td>
            <td class="td_name"><?=$bg_name?></td>
            <td class="td_mbname"><?=$res['hs_name']?></a></td>
            <td class="td_mbid"><?=$mb_id?></td>
            <td class="td_numbig"><?=$res['hs_hp']?></td>
            <td class="td_datetime"><?=$res['hs_datetime']?></td>
            <td class="td_boolean"><?=$res['hs_flag']?'성공':'실패'?></td>
            <td class="td_left">
                <u>결과코드</u> : <?=$res['hs_code']?><br>
                <u>로그</u> : <?=$res['hs_log']?><br>
                <u>메모</u> : <?=$res['hs_memo']?>
            </td>
            <td class="td_mng td_mng_s">
                <?php if ($res['bk_no']) { ?>
                <a href="./history_num.php?wr_id=<?=$res['wr_no']?>&amp;st=bk_no&amp;sv=<?=$res['bk_no']?>" class="btn_03 btn">내역</a>
                <?php } else { ?>
                <a href="./history_num.php?wr_id=<?=$res['wr_no']?>&amp;st=hs_hp&amp;sv=<?=$res['hs_hp']?>" class="btn_03 btn">내역</a>
                <?php } ?>
            </td>
        </tr>
        <?php } ?>
...
<?=sms5_sub_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $spage, $total_spage, $_SERVER['SCRIPT_NAME']."?wr_no=$wr_no&amp;wr_renum=$wr_renum&amp;page=$page&amp;st=$st&amp;sv=$sv&amp;sst=$sst&amp;ssv=$ssv", "", "spage"); ?>
