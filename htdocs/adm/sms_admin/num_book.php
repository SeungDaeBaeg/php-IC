...
<div class="local_ov01 local_ov">
    <span class="btn_ov01"><span class="ov_txt">업데이트 </span><span class="ov_num"><?=isset($sms5['cf_datetime']) ? $sms5['cf_datetime'] : ''; ?></span></span>
    ...
    <span class="btn_ov01"><span class="ov_txt"> 비회원  </span><span class="ov_num"> <?=number_format($no_member_count)?>명</span></span>
...
<select name="bg_no" id="bg_no" onchange="location.href='<?=$_SERVER['SCRIPT_NAME']?>?bg_no='+this.value;">
    <option value=""<?=get_selected('', $bg_no); ?>> 전체 </option>
    <option value="<?=$no_group['bg_no']?>"<?=get_selected($no_group['bg_no'], $bg_no); ?>> <?=$no_group['bg_name']?> (<?=number_format($no_group['bg_count'])?> 명) </option>
    <?php for($i=0; $i<count($group); $i++) {?>
    <option value="<?=$group[$i]['bg_no']?>"<?=get_selected($group[$i]['bg_no'], $bg_no);?>> <?=$group[$i]['bg_name']?> (<?=number_format($group[$i]['bg_count'])?> 명) </option>
    <?php } ?>
</select>
<input type="checkbox" name="no_hp" id="no_hp" <?=$no_hp_checked?> onclick="no_hp_click(this.checked)">
...
<input type="hidden" name="str_query" value="<?=clean_query_string($_SERVER['QUERY_STRING']); ?>" >
...
<div class="tbl_head01 tbl_wrap">
    ...
    <?php if (!$total_count) { ?>
    <tr>
        <td colspan="<?=$colspan?>" class="empty_table">데이터가 없습니다.</td>
    </tr>
    <?php
    }
    $line = 0;
    ...
    <tr class="<?=$bg; ?>">
        <td class="td_chk">
            <label for="bk_no_<?=$i; ?>" class="sound_only"><?=$group_name?>의 <?=get_text($res['bk_name']) ?></label>
            <input type="checkbox" name="bk_no[]" value="<?=$res['bk_no']?>" id="bk_no_<?=$i; ?>">
        </td>
        <td class="td_num"><?=number_format($vnum--)?></td>
        <td><?=$group_name?></td>
        <td class="td_mbname"><?=get_text($res['bk_name']) ?></td>
        <td class="td_numbig"><?=$res['bk_hp']?></td>
        <td class="td_boolean"><?=$res['bk_receipt'] ? '<font color=blue>수신</font>' : '<font color=red>거부</font>'?></td>
        <td class="td_mbid"><?=$res['mb_id'] ? $res['mb_id'] : '비회원'?></td>
        <td class="td_datetime"><?=$res['bk_datetime']?></td>
        <td class="td_mng td_mng_l">
            <a href="./num_book_write.php?w=u&amp;bk_no=<?=$res['bk_no']?>&amp;page=<?=$page?>&amp;bg_no=<?=$bg_no?>&amp;st=<?=$st?>&amp;sv=<?=$sv?>&amp;ap=<?=$ap?>" class="btn btn_03">수정</a>
            <a href="./sms_write.php?bk_no=<?=$res['bk_no']?>" class="btn btn_02">보내기</a>
            <a href="./history_num.php?st=hs_hp&amp;sv=<?=$res['bk_hp']?>" class="btn btn_02">내역</a>
        </td>
    </tr>
    <?php } ?>
    </tbody>
...
<div class="btn_fixed_top">
    ...
    <a href="./num_book_write.php?page=<?=$page?>&amp;bg_no=<?=$bg_no?>" class="btn btn_01">번호추가</a>
</div>
</form>
<script>
...
<?=get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, $_SERVER['SCRIPT_NAME']."?bg_no=$bg_no&amp;st=$st&amp;sv=$sv&amp;ap=$ap&amp;page="); ?>
